...
def parseCmd():
    ...
    Returns:
        dictionary: A dictionary with arguments
    """
    parser = argparse.ArgumentParser(description='ProtHint ' + VERSION + ':\
        Pipeline for generating genome wide footprints of homologous proteins.\
        The set of high confidence hints is generated using default thresholds\
        in print_high_confidence.py script. If you wish to use different\
        filtering criteria, re-run print_high_confidence.py script with custom\
        thresholds.')
    ...
    parser.add_argument('--workdir', type=str, default='.',
                        help='Folder for results and temporary files. If not\
        specified, current directory is used.')
    parser.add_argument('--geneSeeds', type=str, default='',
                        help='Gene prediction seeds in gtf format. If this\
        file is provided, GeneMark-ES run is skipped.')
    parser.add_argument('--geneMarkGtf', type=str, default='',
                        help='Same as --geneSeeds, for backwards\
        compatibility.')
    parser.add_argument('--fungus', default=False, action='store_true',
                        help='Run GeneMark-ES in fungus mode.')
    parser.add_argument('--nonCanonicalSpliceSites', default=False,
                        action='store_true', help='Predict introns with\
        non-canonical splices sites. By default, ProtHint only predicts\
        introns with GT-AG, GC-AG, and AT-AC donor-acceptor sites.')
    parser.add_argument('--diamondPairs', type=str, default='',
                        help='File with "seed gene-protein" hits generated by\
        DIAMOND. If this file is provided, DIAMOND search for protein hits is\
        skipped. The seed genes in this file must correspond to seed genes\
        passed by "--geneSeeds" option. All pairs in the file are used -- \
        option"--maxProteinsPerSeed" is ignored.')
    parser.add_argument('--maxProteinsPerSeed', type=int, default=25,
                        help='Maximum number of protein hits per seed gene.\
        Increasing this number leads to increased runtime and may improve the\
        sensitivity of hints. Decreasing has an opposite effect. Default is\
        set to 25.')
    parser.add_argument('--evalue', type=float, default=0.001,
                        help='Maximum e-value for DIAMOND alignments hits.\
        Default = 0.001')
    parser.add_argument('--minExonScore', type=float, default=25,
                        help='Discard all hints inside/neighboring exons with\
        score lower than minExonScore. Default = 25')
    parser.add_argument('--cleanup', default=False, action='store_true',
                        help='Delete temporary files and intermediate results.\
        Cleanup is turned off by default as it is useful to keep these files\
        for troubleshooting and the intermediate results might be useful on\
        their own.')
    parser.add_argument('--pbs', default=False, action='store_true',
                        help='Run GeneMark-ES and Spaln on pbs.')
    parser.add_argument('--threads', type=int, default=-1,
                        help='Number of threads used by ES, DIAMOND, and\
        Spaln. By default, all available threads are used.')
    parser.add_argument('--version', action='version', version='%(prog)s ' +
                        VERSION)

    parser.add_argument('--prevGeneSeeds', type=str, default='',
                        help='File with gene seeds which were used in the\
        previous iteration. Next iteration of ProtHint is only executed for\
        --geneSeeds which differ from --prevGeneSeeds. --prevSpalnGff is\
        required when this option is used since results from the previous\
        iteration are reused for seeds which do not differ (Gene ids of such\
        hints are updated to match the new seed genes).')
